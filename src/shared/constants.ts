...
/**
 * 角を丸める半径の既定値（描画メートル）。線路がこれを使う。
 *
 * 経路はノードを直線で結んだ折れ線なので、そのままだと分岐や曲がり角に
 * 90 度の角が立つ。この長さぶんだけ手前から曲がり始める。車体が 4.2m なので、
 * 実車が交差点を曲がるときの回転半径（車 1 台強）に合わせてある。
 * 大きくしすぎると角の手前から膨らんで軌道や対向車線からはみ出して見える。
 * 道路は停止線の位置に合わせて別の値を使う（`ROAD_CORNER_RADIUS_M`）。
 */
export const CORNER_RADIUS_M = 4.2;
/**
 * 道路の角を丸める半径（描画メートル）。線路より小さい。
 *
 * 車は交差点の手前で**止まる**。曲線が停止線より手前から始まっていると、
 * 信号待ちや右左折待ちの車が曲がりかけの姿勢のまま固まる ―― 前にも対向にも
 * 車がいないのに、交差点の中で斜めを向いて止まっている画になる
 * （実測: 曲がる直前で止まっている車の 73.5% が曲線の中にいた）。
 *
 * そこで曲線は**停止線から先**だけにする。停止線を下げて避けることもできるが、
 * リンクに 2 台目が入る余地が無くなって街じゅうの行列の容量が減る
 * （実測: 平均通勤 +9%、朝の待ち台数 +29%）。曲がる区間が 8.4m から 5.0m に
 * 縮むぶん向きの変化は速くなるが、車は 1 タイルを 0.27 秒で走り抜けるので
 * 曲がっている最中の見え方はほとんど変わらない。
 */
export const ROAD_CORNER_RADIUS_M = 2.5;

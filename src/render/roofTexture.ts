...
function rollAt(u: number): number {
  let h = 0;
  if (u < 0.66) {
    h += 0.05 * Math.sin(Math.PI * (u / 0.66));
  } else if (u < 0.93) {
    const t = (u - 0.66) / 0.27;
    h += 0.62 * Math.pow(Math.sin(Math.PI * t), 0.62);
  }
  return h - ss(0.90, 1.0, u) * 0.22;
}
...
function build(): void {
  ...
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      ...
      let c = 0.50 + (rollAt(u) + stepAt(v) * 0.35) * 0.52;
      // 葺き足の段差の影。上の段の鼻が落とす影で、これが横線として読める。
      c *= 1 - (1 - ss(0.0, 0.14, v)) * 0.44;
      // 継ぎ目の溝。縦線。桟瓦の並びはここでしか読めない。
      c *= 1 - ss(0.86, 1.0, u) * 0.46;
